...
def get_notes(user_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return [{"id": n.id, "submission_id": n.submission_id, "content": n.content, "timestamp": n.timestamp.isoformat()} for n in notes]

@router.post("/notes")
def create_note(payload: CreateNoteRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    new_note = Note(user_id=current_user.id, submission_id=payload.submission_id, content=payload.content)
    db.add(new_note)
    db.commit()
